...
# Überprüft die positionen der Zahl (Aktuell noch stark Hardcoded)
def check_position(num):
    num_str = str(num)
    for check in str(position_check):
        if num_str[2] == check:
            return False
        if num_str[1] == '0':
            return False
...
def num_loop(min, max):
    for cur_num in range(min, max, 2):
        if check_inclusions(str(cur_num)) and check_divisivility(cur_num) and check_position(cur_num):
